---
title: "ProdMCP-org"
linkTitle: "ProdMCP-org"
description: "Production-grade tooling for the MCP ecosystem — specification, validation, and framework."
type: "landing"
---

<div class="landing-hero">
  <div class="hero-content">
    <span class="hero-badge">Open Source MCP Ecosystem</span>
    <h1 class="hero-title">Build <span class="highlight">Production-Grade</span> MCP Servers</h1>
    <p class="hero-first-ever"><strong><span class="highlight">The 1<sup>st</sup> ever</span> security auditing &amp; conformance scanning for MCP.</strong></p>
    <p class="hero-subtitle">
      A complete open-source ecosystem for the Model Context Protocol — 
      from specification to validation to framework. Ship MCP servers with confidence.
    </p>
    <div class="projects-grid">
      <a href="/docs/prodmcp/" class="project-card project-card--prodmcp" id="card-prodmcp">
        <div class="project-card__logo-wrapper">
          <img src="/images/prodmcp-logo.png" alt="ProdMCP Logo" class="project-card__logo">
        </div>
        <h2 class="project-card__title">ProdMCP</h2>
        <p class="project-card__tagline">
          Unified production framework for REST APIs and MCP servers. Schema-driven development, 
          validation, security, middleware, and auto-generated specs.
        </p>
        <code class="project-card__pip">pip install prodmcp</code>
        <span class="project-card__cta">Explore ProdMCP</span>
      </a>
      <a href="/docs/openmcp-spec/" class="project-card project-card--openmcp" id="card-openmcp">
        <div class="project-card__logo-wrapper">
          <img src="/images/openmcp-logo.png" alt="OpenMCP Logo" class="project-card__logo">
        </div>
        <h2 class="project-card__title">OpenMCP Specification</h2>
        <p class="project-card__tagline">
          The open standard for describing MCP servers — OpenAPI for MCP. 
          Define tools, prompts, resources, and security in a machine-readable format.
        </p>
        <span class="project-card__cta">Explore Specification</span>
      </a>
      <a href="/docs/mcpcrunch/" class="project-card project-card--mcpcrunch" id="card-mcpcrunch">
        <div class="project-card__logo-wrapper">
          <img src="/images/mcpcrunch-logo.png" alt="MCPcrunch Logo" class="project-card__logo">
        </div>
        <h2 class="project-card__title">MCPcrunch</h2>
        <p class="project-card__tagline">
          Security auditing &amp; conformance testing for OpenMCP specs. 
          42Crunch-style scoring with deterministic and LLM-powered analysis.
        </p>
        <code class="project-card__pip">pip install mcpcrunch</code>
        <span class="project-card__cta">Explore MCPcrunch</span>
      </a>
      <a href="/docs/tutorials/" class="project-card project-card--tutorials" id="card-tutorials">
        <div class="project-card__logo-wrapper">
          <span class="project-card__icon">🎓</span>
        </div>
        <h2 class="project-card__title">7 Day Tutorial</h2>
        <p class="project-card__tagline">
          <b>7 Days to Production MCP.</b> A comprehensive Python-based curriculum 
          from foundations to high-availability deployment.
        </p>
        <span class="project-card__cta">Start Learning</span>
      </a>
    </div>
    <div class="features-strip">
      <div class="feature-item">
        <span class="feature-item__icon">🔒</span>
        <span>Security-First</span>
      </div>
      <div class="feature-item">
        <span class="feature-item__icon">📋</span>
        <span>Schema-Driven</span>
      </div>
      <div class="feature-item">
        <span class="feature-item__icon">⚡</span>
        <span>Production-Ready</span>
      </div>
      <div class="feature-item">
        <span class="feature-item__icon">🤖</span>
        <span>Agent-Friendly</span>
      </div>
      <div class="feature-item">
        <span class="feature-item__icon">🧪</span>
        <span>Fully Tested</span>
      </div>
    </div>
  </div>
</div>
